...
class ReplaysController extends Controller
{
  ...
  */
  public function getMatch($name){
    ...
    $gamp->setDocumentPath( '/api/matches/' . $name . '/total' );
    $gamp->sendPageview();
  ...
  */
  public function matchID($crawler, $client, $name){
    ...
    while($crawler->filter('.next')->each(function ($node) {return $node->text();})){
      $crawler = $client->request('GET', 'https://warthunder.com/en/tournament/replay/page/'. $i .'?Filter%5Bkeyword%5D=&Filter%5Bnick%5D='. $name .'&action=search');
      $matchids[] = $crawler->filter('.replay__item')->extract(array('data-replay'));
      $i++;
    }
